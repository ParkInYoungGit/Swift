...
class pageViewController: UIViewController {
    ...
    func makeSingleTouch(){
        // swipe 명령
        let swipeLeft = UISwipeGestureRecognizer(target: self, action: #selector(pageViewController.responseToSwipeGesture(_:)))
                swipeLeft.direction = UISwipeGestureRecognizer.Direction.left
                self.view.addGestureRecognizer(swipeLeft)
        
        let swipeRight = UISwipeGestureRecognizer(target: self, action: #selector(pageViewController.responseToSwipeGesture(_ :)))
